  ...
  async function callGPTAPI(customQuery = "") {
    ...
    let messages = [
      ...
      { role: "user", content: customQuery },
      {
        ...
        content: selectedRecipe
          ? `레시피 "${selectedRecipe}"에 대한 세부 정보를 알려주세요.`
          : customQuery,
      },
    ];
      ...
      messages: [
        {
          role: "system",
          content:
            "너는 친절하며 전 세계 음식을 다룰 수 있는 유명 쉐프로 경력이 20년이 넘어요.",
        },
        {
          role: "system",
          content:
    ...
    try {
      ...
      if (
        ...
        if (!customQuery) {
          const matchedOptions = content
            .match(/"(.*?)"/g)
            ?.map((option) => option.replace(/"/g, "")); //3가지 레시피 제공 저장: recipeOptions
          if (matchedOptions) {
    ...
    <SafeAreaView>
      ...
      {recipeOptions.map((option, index) => (
        <Button
          key={index}
          title={option}
          onPress={() => handleSelectRecipe(option)}
        />
      ))}
      <TextInput
