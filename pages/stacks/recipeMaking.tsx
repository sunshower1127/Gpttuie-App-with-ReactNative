...
export default function RecipeMaking() {
  ...
  const [servings, setServings] = useState(4); // 레시피 인분
  const [ingredients, setIngredients] = useState(["양파", "쌀", "고기", "계란"]);
  const [recipefrom, setRecipeFrom] = useState("한식");
  ...
  async function callGPTAPI(customQuery = "") {
    ...
    let messages = [
      ...
      { role: "system", content: "3가지 레시피를 선택하라고 할 때는 번호가 아닌 안에 레시피 이름을 쌍따음표 사이에 넣어서 제공해줘" },
      { role: "user", content: `${recipefrom}` },
    ...
    try {
      ...
      if (responseData.choices && responseData.choices.length > 0 && responseData.choices[0].message) {
        ...
        if (!customQuery) {
          const matchedOptions = content.match(/"(.*?)"/g)?.map((option) => option.replace(/"/g, ""));
          if (matchedOptions) {
  ...
  return (
    <View>
      ...
      />
      <Button title="질문하기" onPress={() => callGPTAPI()} />
    </View>
  );
}
